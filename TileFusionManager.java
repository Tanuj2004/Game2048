...
public class TileFusionManager {
    ...
    /**
     * Initializes the game, asks the user for the grid size, and sets up the game window.
     */
    public void start() {
        Scanner scanner = new Scanner(System.in);
        System.out.println("Enter grid size (2 to 8): ");
        gridSize = scanner.nextInt();
        if (gridSize < 2 || gridSize > 8) {
            System.out.println("Invalid size. Defaulting to 4x4.");
            gridSize = 4;
        }
        ...
        frame.addKeyListener(new KeyAdapter() {
            public void keyPressed(KeyEvent e) {
                ...
                if (hasMoved) {
                    addNewTile();
                    if (isGameOver()) {
                        int response = JOptionPane.showConfirmDialog(frame, "Game Over! Your score: " + currentScore + "\nDo you want to play again?", "Game Over", JOptionPane.YES_NO_OPTION);
    ...
    /**
     * Checks if the game is over. The game is over when no more moves are possible.
     *
    ...
    private void resetGame() {
        gameBoard = new int[gridSize][gridSize];
        currentScore = 0;
        // Initialize the board with two starting tiles
        gameBoard[0][0] = 2;
        gameBoard[1][1] = 2;
        tileCanvas.updateBoard(gameBoard, currentScore);
    }
